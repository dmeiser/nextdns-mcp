...
class TestAccessControlledClientWriteAccess:
    ...
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", False)
    async def test_allows_write_when_permitted(self, mock_can_write, mock_super_request):
    ...
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", False)
    async def test_denies_write_when_not_permitted(self, mock_can_write, mock_super_request):
    ...
    @pytest.mark.asyncio
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", True)
    @patch("nextdns_mcp.server.can_write_profile", return_value=False)
    async def test_denies_all_writes_in_read_only_mode(self, mock_can_write, mock_super_request):
    ...
    @pytest.mark.asyncio
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", False)
    async def test_allows_create_profile_without_check(self, mock_super_request):
...
class TestAccessControlledClientMethods:
    ...
    @patch("nextdns_mcp.server.can_write_profile", return_value=True)
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", False)
    async def test_put_is_write_operation(self, mock_can_write, mock_super_request):
    ...
    @patch("nextdns_mcp.server.can_write_profile", return_value=True)
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", False)
    async def test_patch_is_write_operation(self, mock_can_write, mock_super_request):
        """Test that PATCH is treated as a write operation."""
        async with AccessControlledClient(base_url="https://api.nextdns.io") as client:
            response = await client.request("PATCH", "/profiles/abc123/settings", json={})
    ...
    @patch("nextdns_mcp.server.can_write_profile", return_value=True)
    @patch("nextdns_mcp.server.NEXTDNS_READ_ONLY", False)
    async def test_delete_is_write_operation(self, mock_can_write, mock_super_request):
